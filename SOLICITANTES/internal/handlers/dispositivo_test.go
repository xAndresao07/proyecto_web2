...
func TestCrearDispositivo(t *testing.T) {
	...
	t.Run("valido -> 201", func(t *testing.T) {
		body := `{"id":100, "solicitante_id":1, "marca":"HP", "modelo":"Pavilion", "tipo_almacenamiento":"SSD", "ram_gb":8, "sistema_operativo":"Windows"}`
...
func TestObtenerDispositivos(t *testing.T) {
	...
	// Crear uno manual
	creado := almacen.CrearDispositivo(models.Dispositivo{SolicitanteID: 1, Marca: "Dell", Modelo: "XPS", TipoAlmacenamiento: "NVMe", RamGB: 16, SistemaOperativo: "Linux"})
...
func TestActualizarDispositivo(t *testing.T) {
	h, almacen, _ := construirEntorno()
	token := tokenValido(t, h)
	creado := almacen.CrearDispositivo(models.Dispositivo{SolicitanteID: 1, Marca: "Asus", Modelo: "ROG", TipoAlmacenamiento: "SSD", RamGB: 16, SistemaOperativo: "Windows"})
	idStr := strconv.Itoa(int(creado.ID))

	t.Run("valido -> 200", func(t *testing.T) {
		body := `{"id":`+idStr+`, "solicitante_id":1, "marca":"Asus", "modelo":"TUF", "tipo_almacenamiento":"SSD", "ram_gb":16, "sistema_operativo":"Windows"}`
		rec := ejecutar(h, jsonReq(http.MethodPut, "/api/v1/dispositivos/"+idStr, body, token))
		assert.Equal(t, http.StatusOK, rec.Code)
	})

	t.Run("no encontrado -> 404", func(t *testing.T) {
		body := `{"id":999, "solicitante_id":1, "marca":"Asus", "modelo":"TUF", "tipo_almacenamiento":"SSD", "ram_gb":16, "sistema_operativo":"Windows"}`
